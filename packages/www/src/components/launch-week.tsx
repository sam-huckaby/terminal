...
const LaunchWeek: Component<LaunchWeekProps> = ({ children }) => {
  ...
  return (
    <AnimationContext.Provider value={{ registerEvent }}>
      <div class="bg-black min-h-[calc(100vh-40px)]" style={{
        "background-image": "linear-gradient(#180800 4px, transparent 4px);",
        "background-size": "8px 8px;"
      }}>
        {/* Main content */}
        <div class="relative flex flex-col mx-auto max-w-[720px] my-14 md:my-24">
          <div class="flex flex-col justify-center px-8 md:px-0">
            {/* Title section */}
            <div class="text-white flex flex-col md:flex-row justify-between items-start md:items-center gap-2 md:gap-12 mb-20 md:mb-12 ">
              <h1 class="text-5xl font-medium md:text-[74px] tracking-[-1.48px] leading-[180%]">
                launch week
              </h1>
              <div class="border-2 border-white rounded-2xl px-5 md:px-8">
                <span class="text-2xl md:text-[40px] tracking-[-0.8px] leading-[180%]">24.03</span>
              </div>
            </div>

            {/* Events list */}
            <div class="flex flex-col gap-10 md:gap-6">
              <EventItem
                date="24.03"
                title="experiences beyond the terminal"
              />
              <EventItem
                date="25.03"
          ...
          <div class="pointer-events-none -mx-20 md:-mx-36">
            {children}
          </div>
          ...
          <div class="w-full flex flex-col items-center">
            ...
            <p class="text-xs text-center text-[#9C9C9C] mt-6">
...
const EventItem: Component<{ date: string; time?: string; keyword?: string; title: string; href?: string }> = (props) => {
  ...
  return (
    <div class="flex flex-col md:flex-row md:items-center md:gap-8 text-[#9C9C9C] text-lg font-medium">
      <span class="tracking-[-0.36px]">
        {props.date}
      </span>
      <div class="flex-1 flex flex-col md:flex-row md:items-center gap-1 mb-1 md:gap-2 md:mb-0">
        <span classList={{
          "tracking-[-0.36px]": true,
          'text-orange': !!props.keyword,
          'transition-all duration-300': true,
          'font-mono': !animationEnded(),
        }}>
          {animatedText()}
        </span>
